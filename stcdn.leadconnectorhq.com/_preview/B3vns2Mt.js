...
const ne = () => {
    ...
    function d(e, t = !1, n) {
        ...
        try {
            ...
            if (!r || !r.id) return;
            let s = r.url;
            v(c.path) && (s = `${g(c.path)}/${r.id}`), n && (s += `/product/${n}`);
            const f = a.value.funnelDomain;
            if (window.location.hostname !== "localhost" && !v(c.path) && f !== window.location.hostname) {
                const u = `https://${f}${s}` + h();
                w({
                    url: u,
                    newTab: t
    ...
    function w({
        ...
        a.value.videoExistsInPage && $.$emit("handle-redirect"), e = M(e);
    ...
    const T = async ({
        ...
        p = (e, t = !0, n) => {
            try {
                ...
                if (v(c.path)) {
                    let i = "";
                    return r && (i = `${g(c.path)}/${r.id}`), n && (i += `/product/${n}`), i
                }
                let s = r.url;
                const f = a.value.funnelDomain;
                n && (s += `/product/${n}`);
                const u = new URL(s, `https://${f}`);
                if (t) {
                    const i = { ...c.query
                    };
        ...
        _ = e => {
            ...
            return !t || v(c.path) ? "" : `https://${a.value.funnelDomain}${t.url}/store/account/${e}`
        },
        y = e => {
            const t = a.value.funnelSteps ? .find(n => n ? .key === "store-product-list");
            if (!t || v(c.path)) return "";
            w({
            ...
            try {
                ...
                if (r) alert(s);
                    ...
                    const i = JSON.parse(localStorage.getItem("_ud") || "{}"),
                        l = [z.sepa].includes(localStorage.getItem(`pmt_${t.value.funnelId}_${i?.id}`) || "") ? "We are proccessing your order with the bank. We'll confirm you order after bank's confirmation." : `Order successfully placed! 
 However, Something went wrong while displaying this webpage, please contact the seller.`;
                    alert(l)
                }
            }
        },
        getEcomProductListPageUrl: A,
        goToStep: U,
        ...
        getEcomProductListPagePath: e => {
            ...
            if (!t) return "";
            let n = "";
            return v(c.path) ? n = `${g(c.path)}/${t?.id}` : n = `https://${a.value.funnelDomain}${t.url}`, e && (n += `/collections/${e}`), n
        },
